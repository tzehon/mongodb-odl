...
async def get_sync_status(
    ...
    latest = await collection.find_one(
        {},
        sort=[("statementMetadata.generatedAt", -1)],
        projection={
            "accountNumber": 1,
            "statementMetadata": 1,
            "_updated_at": 1
    ...
    # Calculate sync lag
    if latest and latest.get("statementMetadata", {}).get("generatedAt"):
        last_sync = latest["statementMetadata"]["generatedAt"]
        if isinstance(last_sync, datetime):
            sync_lag_seconds = (datetime.utcnow() - last_sync).total_seconds()
        else:
    ...
    return {
        ...
        "lastSyncedDocument": {
            ...
            "generatedAt": last_sync.isoformat() if last_sync else None,
            "source": latest.get("statementMetadata", {}).get("source") if latest else None
        },
        "syncLagSeconds": round(sync_lag_seconds, 2) if sync_lag_seconds else None,
        "timestamp": datetime.utcnow().isoformat()
    }
